...
class DiplomaVerify extends Component {
	...
	handleSelect = (ra) => {
		...
		const newCheckeds = checkeds.map(
			(diploma) => (diploma.RA === ra ? { ...diploma, check: !diploma.check } : diploma)
		);
		this.setState({
	...
	render() {
		const { checkeds, selectAll, loading } = this.state;
		return (
			<LoadingScreen loading={loading} bgColor="#FFF" spinnerColor="#ED3B48">
				...
				<UIButton onClick={this.handleSubmit}>Enviar</UIButton>
			</LoadingScreen>
		);
	}
}
